...
function GetCSharpCoreCommand($configuration = "Debug")
{
    ...
    return @(
        ...
        "`"C:\ProgramData\chocolatey\bin\OpenCover.Console.exe`""
        "-target:$dotnet"
        "-targetargs:test --filter TestCategory!=SlowCoverage&TestCategory!=SkipCI `"$(Join-PathEx $root PrtgAPI.Tests.UnitTests PrtgAPIv17.Tests.UnitTests.csproj)`" --verbosity:n --no-build -c $configuration"
        "-output:`"$($temp)opencover.xml`""
        "-filter:+`"[PrtgAPI*]* -[PrtgAPI.Tests*]*`""
        "-excludebyattribute:System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverageAttribute"
        "-hideskipped:attribute"
...
function GetCSharpFullCommand($configuration = "Debug")
{
    ...
    $expected = @(
        "`"C:\ProgramData\chocolatey\bin\OpenCover.Console.exe`""
        "-target:C:\vstest.console.exe"
        "-targetargs:/TestCaseFilter:TestCategory!=SlowCoverage&TestCategory!=SkipCI \`"$(Join-PathEx $root PrtgAPI.Tests.UnitTests bin $configuration PrtgAPI.Tests.UnitTests.dll)\`""
        "-output:`"$($temp)opencover.xml`""
        "-filter:+`"[PrtgAPI*]* -[PrtgAPI.Tests*]*`""
        "-excludebyattribute:System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverageAttribute"
        "-hideskipped:attribute"
...
Describe "Get-PrtgCoverage" -Tag @("PowerShell", "Build") {
    ...
    It "filters to specific C# tests" {
        ...
        $expected1 = @(
            ...
            "`"C:\ProgramData\chocolatey\bin\OpenCover.Console.exe`""
            "-target:$dotnet"
            "-targetargs:test --filter TestCategory!=SlowCoverage&TestCategory!=SkipCI&FullyQualifiedName~dynamic `"$(Join-PathEx $root PrtgAPI.Tests.UnitTests PrtgAPIv17.Tests.UnitTests.csproj)`" --verbosity:n --no-build -c Debug"
            "-output:`"$($temp)opencover.xml`""
            "-filter:+`"[PrtgAPI*]* -[PrtgAPI.Tests*]*`""
            "-excludebyattribute:System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverageAttribute"
            "-hideskipped:attribute"
