...
{
    class TakeFilteredSensorsInsufficientOneLeftScenario : TakeScenario
    {
        protected override IWebResponse GetResponse(string address, Content content)
        {
            switch (requestNum)
            {
                ...
                case 2: //We're going to have to stream. Request how many objects exist
                    ...
                    return new SensorResponse(Enumerable.Range(0, 3).Select(i => new SensorItem()).ToArray());

                case 3: //Request the 1 remaining sensor
                    Assert.AreEqual(TestHelpers.RequestSensor("count=1&filter_name=ping&start=2", UrlFlag.Columns), address);
