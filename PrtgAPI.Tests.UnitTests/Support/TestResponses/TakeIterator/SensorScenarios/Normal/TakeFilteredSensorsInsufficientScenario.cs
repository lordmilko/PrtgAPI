        ...
        protected override IWebResponse GetResponse(string address, Content content)
        {
            switch (requestNum)
            {
                ...
                case 3: //Request the next 2 sensors
                    Assert.AreEqual(TestHelpers.RequestSensor("count=2&filter_name=ping&start=2", UrlFlag.Columns), address);
                ...
                case 4: //Still haven't gotten all the sensors we want. Ask for 500 sensors then
                    Assert.AreEqual(TestHelpers.RequestSensor("count=500&filter_name=ping&start=4", UrlFlag.Columns), address);
                ...
                case 5: //Still haven't gotten all the sensors we want. Ask for the remaining 96
                    Assert.AreEqual(TestHelpers.RequestSensor("count=96&filter_name=ping&start=504", UrlFlag.Columns), address);
                    return new SensorResponse(Enumerable.Range(0, 600).Select(i => new SensorItem()).ToArray());
