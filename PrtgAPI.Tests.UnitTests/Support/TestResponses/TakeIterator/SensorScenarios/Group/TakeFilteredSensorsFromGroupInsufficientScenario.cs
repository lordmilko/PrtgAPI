        ...
        protected override IWebResponse GetResponse(string address, Content content)
        {
            switch (requestNum)
            {
                ...
                case 3: //Get all sensors under the group "Windows Infrastructure"
                    Assert.AreEqual(TestHelpers.RequestSensor("count=*&filter_name=@sub(ping)&filter_group=Windows+Infrastructure", UrlFlag.Columns), address);
                ...
                case 6: //Get all sensors under the group "Child Group"
                    Assert.AreEqual(TestHelpers.RequestSensor("count=*&filter_name=@sub(ping)&filter_group=Child+Group", UrlFlag.Columns), address);
                ...
                case 7: //Get the child groups of "Child Group"
                    Assert.AreEqual(TestHelpers.RequestGroup("count=*&filter_parentid=2001", UrlFlag.Columns), address);
                ...
                case 8: //Are there any other groups called "Grandchild Group"?
                    Assert.AreEqual(TestHelpers.RequestGroup("count=*&filter_name=Grandchild+Group", UrlFlag.Columns), address);
                    return new GroupResponse(new GroupItem(name: "Grandchild Group", objid: "2002", totalsens: "0"));
