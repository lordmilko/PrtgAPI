        ...
        protected override IWebResponse GetResponse(string address, Content content)
        {
            switch (requestNum)
            {
                ...
                case 3: //Get all devices under the group "Windows Infrastructure" that we're piping from
                    Assert.AreEqual(TestHelpers.RequestDevice("count=*&filter_parentid=2000", UrlFlag.Columns), address);
                ...
                case 4: //Get all sensors from the child devices
                    Assert.AreEqual(TestHelpers.RequestSensor("count=*&filter_name=@sub(ping)&filter_parentid=3000&filter_parentid=3001", UrlFlag.Columns), address);
                ...
                case 5: //Get all child groups under "Windows Infrastructure
                    Assert.AreEqual(TestHelpers.RequestGroup("count=*&filter_parentid=2000", UrlFlag.Columns), address);
                ...
                case 7: //Get all devices under the group "Child Group" that we're piping from
                    Assert.AreEqual(TestHelpers.RequestDevice("count=*&filter_parentid=2001", UrlFlag.Columns), address);
                ...
                case 8: //Get all sensors from the child devices
                    Assert.AreEqual(TestHelpers.RequestSensor("count=*&filter_name=@sub(ping)&filter_parentid=3002&filter_parentid=3003", UrlFlag.Columns), address);
