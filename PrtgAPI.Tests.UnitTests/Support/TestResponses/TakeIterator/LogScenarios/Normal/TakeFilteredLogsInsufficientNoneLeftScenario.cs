...
{
    class TakeFilteredLogsInsufficientNoneLeftScenario : TakeScenario
    {
        protected override IWebResponse GetResponse(string address, Content content)
        {
            switch (requestNum)
            {
                ...
                case 3: //Request the 1 remaining log
                    Assert.AreEqual(TestHelpers.RequestLog("count=1&start=3&filter_name=ping", UrlFlag.Columns), address);
