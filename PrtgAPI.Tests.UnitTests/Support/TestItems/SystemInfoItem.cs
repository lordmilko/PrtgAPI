...
namespace PrtgAPI.Tests.UnitTests.Support.TestItems
{
    public class SystemInfoItem
    {
        ...
        public static SystemInfoItem SystemItem(string key = "\"IPAddress / vmxnet3 Ethernet Adapter\"",
            ...
            string displayName = "\"IPAddress#2: fdd6:8a79:a465:d577:1c60:344f:6714:d010\"")
        {
            var content = MakeObject(key, value, id, adapter, displayName);
        ...
        public static SystemInfoItem HardwareItem(string name = "\"\\\\\\\\.\\\\PHYSICALDRIVE0\"", string description = "\"Disk drive\"",
            ...
            string serialNumber = "\"\"", string capacity = "\"\"", string displayName = "\"\\\\\\\\.\\\\PHYSICALDRIVE0 (Disk drive)\"")
        {
            var content = MakeObject(name, description, @class, caption, state, serialNumber, capacity, displayName);
        ...
        public static SystemInfoItem SoftwareItem(string name = "\"Configuration Manager Client\"", string vendor = "\"Microsoft Corporation\"",
            ...
            string displayName = "\"Microsoft Corporation Configuration Manager Client\"")
        {
            var content = MakeObject(name, vendor, version, date, size, displayName);
        ...
        public static SystemInfoItem ProcessItem(string processId = "1044", string caption = "\"WmiPrvSE.exe\"",
            string creationDate = "\"2018-08-31 19:36:26\"", string displayName = "\"WmiPrvSE.exe\"")
        {
            var content = MakeObject(processId, caption, creationDate, displayName);
        ...
        public static SystemInfoItem ServiceItem(string name = "\"wuauserv\"", string description = "\"Enables the detection, download, and installation of updates for Windows and other programs. If this service is disabled, users of this computer will not be able to use Windows Update or its automatic updating feature, and programs will not be able to use the Windows Update Agent (WUA) API.\"",
            ...
            string displayName = "\"wuauserv\"")
        {
            var content = MakeObject(name, description, startName, startMode, state, displayName);
        ...
        public static SystemInfoItem UserItem(string domain = "\"PRTG-1\"", string user = "\"NETWORK SERVICE\"", string displayName = "\"PRTG-1\\\\NETWORK SERVICE\"")
        {
            var content = MakeObject(domain, user, displayName);
        ...
        {
            var pairs = properties.Where(p => p != null).Select(p => $"\"\":{p}");
